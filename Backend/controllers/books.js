...
const getAllBooks = asyncWrapper(async (req, res) => {
    ...
    return res.status(202).send({message: "Başarılı", data: books, count: books.length})
})

const getBook = asyncWrapper(async (req, res) => {
    const book = await Book.findById(req.params.id);
    if (!book) {
        return res.status(404).send({message: "Kayıtbulunamadı"})
        }
        return res.status(202).send({message: "Başarılı", data: book})
        })

const addBook = asyncWrapper(async (req, res) => {
    if (!req.body.title || !req.body.author) {
        return res.status(400).send({message: "Tüm alanları doldurunuz (title ve author gerekli)"})
        }
...
const deleteBook = asyncWrapper(async (req, res) => {
    ...
    if (!book) {
        ...
        return res.status(200).send({message: "Başarılı", data: book})
})
